...
.PHONY: help check-runtime lint lint-shell lint-python lint-config lint-rules test test-python validate build-custom-rules build-custom-rules-text clean
...
help:
	...
	@echo "  make test              Run all repository test scripts (includes the Python suite)"
	@echo "  make test-python       Run only the Python unit test suite"
	@echo "  make validate          Run lint and tests"
	@echo "  make build-custom-rules      Build custom rules and binary artifacts (Linux only)"
	@echo "  make build-custom-rules-text Build custom text artifacts without downloading binary compilers"
...
build-custom-rules:
	./scripts/commands/build-custom-rules.sh

build-custom-rules-text:
	RULES_BUILD_CUSTOM_TEXT_ONLY=1 ./scripts/commands/build-custom-rules.sh
