...
每日定时（或 push 到 main 后）：域名主干取 [v2fly/domain-list-community](https://github.com/v2fly/domain-list-community) 的最新 master，加上 ShellCrash 的 `fakeip-filter`；IP 侧 cn 由多源合并（APNIC、Clang.CN、gaoyifan/china-operator-ip、chnroutes2 BGP、17mon/iPIP），google、telegram、apple 均为 [GeoLite2-ASN](https://github.com/Loyalsoldier/geoip) 注册库按 `asn_groups` 提取的纯 ASN 口径。数据经规范化、去重和上游健康检查（条目数低于阈值直接中止）后，按各客户端原生格式渲染，用 sing-box / mihomo 官方编译器生成二进制产物，守卫校验（最小条目数、CIDR 有效性）通过后五个分支一次性原子推送。sing-box / mihomo 编译器版本与 GitHub Actions 每周自动检测升级，验证编译通过后自动提 PR 合并。
