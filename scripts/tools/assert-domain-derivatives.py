...
def main() -> int:
    ...
    parser.add_argument("min_ads", type=int)
    ...
    print(
        ...
        f"@ads={len(ads_attr)} (min {args.min_ads}), "
    ...
    if len(ads_attr) < args.min_ads:
        errors.append(f"@ads derivative rule sets too low: {len(ads_attr)} < {args.min_ads}")
