...
# Attributes published as standalone derivative rule sets. The @ads attribute
# still drives base-list filtering (e.g. cn excludes ad entries) but its
# derivatives are not shipped as separate artifacts.
...
def export_data_dir_lists(data_dir: Path, output_dir: Path) -> None:
    ...
    for name in names:
        ...
        for attr, rules in sorted(rule_set_output.rules_by_attr.items()):
            if attr not in PUBLISHED_ATTRS or is_redundant_attr_rule_set_name(name, attr):
                continue
...
def main() -> int:
    ...
    handlers = {
        "export": lambda: export_lists(Path(args.data_dir), Path(args.output_dir)),
