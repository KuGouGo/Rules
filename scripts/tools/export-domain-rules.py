...
def selected_rule_set_names(
    ...
    common_not_cn = policy.geolocation_not_cn
    geographic_roots = policy.geographic_roots
    if sorted(geographic_roots - all_names):
        raise ValueError(
            "publish policy geographic roots were removed or renamed upstream: "
            + ", ".join(sorted(geographic_roots - all_names))
        )

    missing = sorted(common_not_cn - all_names)
    if missing:
        raise ValueError(
    ...
    invalid = sorted(common_not_cn - not_cn_reachable)
    if invalid:
        raise ValueError(
    ...
    missing_standalone = sorted(policy.standalone - all_names)
    moved_not_cn = sorted(policy.standalone & not_cn_reachable)
    moved_cn = sorted(
        policy.standalone & reachable_rule_set_names(include_rules, GEO_CN)
    )
    ...
    return set(policy.standalone) | common_not_cn | core | geographic_roots
...
def export_data_dir_lists(
    ...
    if policy:
        replacement_selected = selected_rule_set_names(all_names, include_rules, policy)
        for alias, target in policy.compatibility_replacements.items():
            ...
            if alias in replacement_selected or target not in replacement_selected:
                raise ValueError(
                    "compatibility replacement must omit the alias and publish its "
                    f"target: {alias} -> {target}"
                )
            target_keys = {
                (rule.kind, rule.value)
    ...
    selected = selected_rule_set_names(all_names, include_rules, policy)
...
def export_lists(
    ...
    if input_path.is_dir():
        export_data_dir_lists(input_path, output_dir, publish_policy)
        return
    if publish_policy:
        raise ValueError("publish policy is supported only for DLC data directories")
