...
def normalized_semantic_entries(entries: Counter[RuleEntry]) -> Counter[RuleEntry]:
    ...
    covering_rules = {("DOMAIN-SUFFIX", suffix) for suffix in minimal_suffixes}
    result = Counter({("DOMAIN-SUFFIX", suffix): 1 for suffix in minimal_suffixes})
    for (kind, value), count in entries.items():
        if kind == "DOMAIN-SUFFIX":
            continue
        if kind == "DOMAIN" and domain_rule_is_covered(kind, value, covering_rules):
            continue
...
def canonicalize_cidr_value(canonical_kind: str, raw_value: str, line_number: int) -> str:
    ...
    if canonical_kind != expected_kind or raw_value != value:
...
def verify_classical(path: Path, capability: Any, platform: str, artifact_type: str) -> Counter[RuleEntry]:
    ...
    for line_number, line in enumerate(lines, 1):
        ...
        if artifact_type == "domain":
            ...
            value = canonicalize_cidr_value(canonical_kind, value, line_number)
