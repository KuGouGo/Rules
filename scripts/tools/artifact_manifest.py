...
TOP_KEYS = {
    "schema_version", "generation_id", "build_id", "build_scope",
    "source", "baseline", "inputs", "tools", "summaries",
    "artifacts", "restoration", "publication_profile",
}
SOURCE_KEYS = {"commit", "tree"}
INPUT_KEYS = {"capabilities", "tool_lock", "artifact_origins", "domain_publish_policy"}
INPUT_VALUE_KEYS = {"path", "sha256"}
TOOLS_KEYS = {"lock", "installed"}
SUMMARY_KEYS = {"path", "sha256", "content"}
ARTIFACT_KEYS = {
    "path", "platform", "type", "extension",
    "bytes", "sha256", "origin", "verification",
}
...
def collect_artifacts(root: Path, origins: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for (artifact_type, platform), details in sorted(matrix.items()):
        ...
        for path in sorted(directory.iterdir(), key=lambda item: item.name):
            ...
            artifacts.append({
                "path": rel, "platform": platform, "type": artifact_type,
                "extension": details["extension"], "bytes": path.stat().st_size,
                "sha256": digest(path), "origin": origins[rel],
...
def validate_verification(value: Any, location: str, errors: list[str]) -> None:
    keys = {
        "status", "method", "decoded_counts", "decoded_count",
        "decoded_semantic_sha256", "canonical_linkage",
    }
    ...
    if not isinstance(counts, dict) or any(
        ...
        type(value["decoded_count"]) is not int
        or value["decoded_count"] != sum(counts.values())
        or value["decoded_count"] <= 0
    ):
        errors.append(f"{location}.decoded_count is invalid")
    if not isinstance(value["decoded_semantic_sha256"], str) or not SHA256_RE.fullmatch(
        value["decoded_semantic_sha256"]
    ):
    ...
    if not isinstance(linkage, dict) or linkage.get("status") not in {"matched", "unavailable"}:
        ...
        if (
            set(linkage) != {"status", "source", "counts", "semantic_sha256"}
            or not isinstance(linkage.get("source"), str)
            or not isinstance(linkage.get("counts"), dict)
            or not isinstance(linkage.get("semantic_sha256"), str)
            or not SHA256_RE.fullmatch(linkage["semantic_sha256"])
        ):
            errors.append(f"{location}.canonical_linkage matched schema is invalid")
    elif (
        set(linkage) != {"status", "reason"}
        or not isinstance(linkage.get("reason"), str)
        or not linkage["reason"]
    ):
...
def validate_tool_provenance(root: Path, tools: Any, lock: dict[str, Any], errors: list[str]) -> None:
    ...
    for name, sidecar in installed.items():
        ...
        expected_keys = {
            "schema_version", "tool", "version", "tag_commit",
            "platform", "asset", "archive_sha256",
            "binary_sha256", "version_probe",
        }
        ...
        platform_lock = (
            locked.get("platforms", {}).get(platform)
            if isinstance(platform, str) else None
        )
        expected = {
            "schema_version": 1, "tool": name,
            "version": locked.get("version"),
            "tag_commit": locked.get("tag_commit"),
        }
        if any(
            sidecar.get(key) != value for key, value in expected.items()
        ) or not isinstance(platform_lock, dict):
            errors.append(f"tool provenance disagrees with lock: {name}")
            continue
        if (
            sidecar.get("asset") != platform_lock.get("asset")
            or sidecar.get("archive_sha256") != platform_lock.get("sha256")
            or sidecar.get("binary_sha256") != platform_lock.get("binary_sha256")
        ):
...
def validate_publication_cohort(value: Any, location: str, errors: list[str]) -> bool:
    ...
    if not isinstance(branches, dict) or set(branches) != PUBLISH_BRANCHES:
        ...
        for branch, item in branches.items():
            if not require_exact(item, BRANCH_KEYS, f"{location}.branches.{branch}", errors):
...
def generate(args: argparse.Namespace) -> None:
    ...
    capabilities_path = root / "config/domain-platform-capabilities.json"
    publish_policy_path = root / "config/domain-publish-policy.json"
    ...
    for name in ("upstream-summary.json", "build-summary.json"):
        path = output / name
        if path.is_file():
            summaries[name.removesuffix(".json").replace("-", "_")] = {
                "path": name, "sha256": digest(path),
                "content": load_json(path),
            }
    baseline_default = root / ".tmp" / "publication-baseline.json"
    baseline_path = Path(os.environ.get("ARTIFACT_BASELINE_FILE", baseline_default))
    try:
    ...
    if baseline["status"] == "consistent" and source_commit:
        baseline_source = baseline["source_commit"]
        if not git_commit_exists(root, baseline_source):
            print(
                f"warning: publication baseline source {baseline_source} is unavailable "
                "locally; recording baseline as-is for recovery republish",
                file=sys.stderr,
            )
        elif not git_is_ancestor(root, baseline_source, source_commit):
            raise SystemExit(
                f"stale source refused: publication baseline {baseline_source} "
                f"is not an ancestor of candidate {source_commit}"
            )
    restoration_path = output / "restoration-metadata.json"
    restoration = load_json(restoration_path) if restoration_path.is_file() else None
    ...
    manifest = {"schema_version": SCHEMA_VERSION, "generation_id": args.generation_id,
                ...
                "inputs": {
                    "capabilities": {
                        "path": "config/domain-platform-capabilities.json",
                        "sha256": digest(capabilities_path),
                    },
                    "tool_lock": {
                        "path": "config/tools-lock.json",
                        "sha256": digest(lock_path),
                    },
                    "artifact_origins": {
                        "path": "artifact-origins.json",
                        "sha256": digest(origins_path),
                    },
                    "domain_publish_policy": {
                        "path": "config/domain-publish-policy.json",
                        "sha256": digest(publish_policy_path),
                    },
                },
...
def _validate_manifest_header(manifest: dict, errors: list[str]) -> None:
    ...
    if manifest.get("build_scope") not in {"custom", "full"}:
        errors.append("manifest build_scope must be custom or full")
    if manifest.get("publication_profile") not in {"common", "extended"}:
        errors.append("manifest publication_profile must be common or extended")


def _validate_manifest_source(source: Any, root: Path, source_sha: str | None, errors: list[str]) -> None:
    if not require_exact(source, SOURCE_KEYS, "manifest source", errors):
        return
    for key in SOURCE_KEYS:
        if source[key] is not None and (
            not isinstance(source[key], str)
            or not GIT_SHA_RE.fullmatch(source[key])
        ):
            errors.append(f"manifest source.{key} must be a Git object id or null")
    if source_sha:
        actual_tree = git_value(root, "rev-parse", f"{source_sha}^{{tree}}")
        if actual_tree is None:
            errors.append(f"source SHA is not a locally resolvable Git commit: {source_sha}")
        if source["commit"] != source_sha:
            errors.append(f"source commit mismatch: expected {source_sha}, got {source['commit']}")
        if actual_tree and source["tree"] != actual_tree:
            errors.append(f"source tree mismatch: expected {actual_tree}, got {source['tree']}")


def _validate_manifest_baseline(baseline: Any, source: Any, root: Path, errors: list[str]) -> None:
    if not validate_publication_cohort(baseline, "manifest baseline", errors):
        return
    ...
    if (
        ...
        and git_commit_exists(root, baseline_source)
...
def _validate_manifest_inputs(
    inputs: Any, paths: list[tuple[str, Path, str]], errors: list[str]
) -> None:
    if not require_exact(inputs, INPUT_KEYS, "manifest inputs", errors):
        return
    for name, path, recorded_path in paths:
        item = inputs[name]
        if not require_exact(item, INPUT_VALUE_KEYS, f"manifest inputs.{name}", errors):
            continue
        if item["path"] != recorded_path:
            errors.append(f"{name} input path mismatch")
        if not path.is_file() or item["sha256"] != digest(path):
            errors.append(f"{name} hash mismatch")


def _validate_manifest_summaries(
    summaries: Any, output: Path, errors: list[str]
) -> None:
    if not isinstance(summaries, dict) or set(summaries) - {"upstream_summary", "build_summary"}:
        errors.append("manifest summaries has unsupported keys")
        return
    if "build_summary" not in summaries:
        errors.append("manifest summaries must include build_summary")
        return
    for key, item in summaries.items():
        if not require_exact(item, SUMMARY_KEYS, f"manifest summaries.{key}", errors):
            continue
        expected_path = key.replace("_", "-") + ".json"
        if item["path"] != expected_path:
            errors.append(f"summary path mismatch: {key}")
            continue
        path = output / expected_path
        try:
            content = load_json(path)
        except (OSError, json.JSONDecodeError) as exc:
            errors.append(f"summary unreadable: {expected_path}: {exc}")
            continue
        if item["sha256"] != digest(path):
            errors.append(f"summary hash mismatch: {expected_path}")
        if item["content"] != content:
            errors.append(f"summary embedded content mismatch: {expected_path}")


def _validate_manifest_restoration(
    restoration: Any, build_scope: str, baseline: Any, errors: list[str]
) -> None:
    if build_scope == "custom":
        ...
        errors.append("full manifest restoration must be null")


def _validate_artifact_entry(
    entry: Any, index: int, root: Path, output: Path,
    matrix: dict[tuple[str, str], dict[str, str]],
    origins: dict[str, str], errors: list[str],
) -> str | None:
    if not require_exact(entry, ARTIFACT_KEYS, f"artifact entry {index}", errors):
        return None
    rel = entry["path"]
    if not isinstance(rel, str) or not safe_publish_path(rel):
        errors.append(f"unsafe or nested artifact path: {rel!r}")
        return None
    artifact_type, platform, filename = PurePosixPath(rel).parts
    expected = matrix.get((artifact_type, platform))
    extension = filename.rsplit(".", 1)[-1] if "." in filename else ""
    if not expected:
        errors.append(f"artifact capability missing for {rel}")
    elif (
        entry["type"] != artifact_type
        or entry["platform"] != platform
        or entry["extension"] != extension
        or extension != expected["extension"]
    ):
        errors.append(f"artifact metadata disagrees with capabilities: {rel}")
    if entry["origin"] not in ALLOWED_ORIGINS or origins.get(rel) != entry["origin"]:
        errors.append(f"artifact origin provenance mismatch for {rel}")
    ...
    if size <= 0:
        errors.append(f"zero-byte artifact: {rel}")
    if type(entry["bytes"]) is not int or entry["bytes"] != size:
        errors.append(f"artifact byte count mismatch: {rel}")
    if (
        not isinstance(entry["sha256"], str)
        or not SHA256_RE.fullmatch(entry["sha256"])
        or entry["sha256"] != digest(path)
    ):
    ...
    try:
        actual_verification = verify_one(root, path, artifact_type, platform)
    except Exception as exc:
        errors.append(f"artifact binary/readability verification failed for {rel}: {exc}")
    else:
        if entry["verification"] != actual_verification:
            errors.append(f"artifact verification metadata mismatch: {rel}")
    return rel


def _collect_actual_artifacts(
    output: Path, matrix: dict[tuple[str, str], dict[str, str]], errors: list[str]
) -> set[str]:
    actual: set[str] = set()
    for artifact_type in ("domain", "ip"):
        ...
        for path in base.rglob("*"):
            if not path.is_file():
                continue
            rel = path.relative_to(output).as_posix()
            parts = PurePosixPath(path.relative_to(output).as_posix()).parts
            if rel in INTERNAL_ARTIFACT_FILES:
                continue
            if len(parts) != 3:
                errors.append(f"unexpected nested publishable file: {rel}")
                continue
            expected = matrix.get((parts[0], parts[1]))
            if not expected or path.suffix != "." + expected["extension"]:
                errors.append(f"unexpected publishable file: {rel}")
                continue
            actual.add(rel)
    return actual
...
def verify(args: argparse.Namespace) -> None:
    ...
    _validate_manifest_source(source, root, args.source_sha, errors)
    baseline = manifest.get("baseline")
    _validate_manifest_baseline(baseline, source, root, errors)
    ...
    try:
        matrix, lock, origins = (
            capability_matrix(root),
            load_json(lock_path),
            load_origin_file(origins_path, require_nonempty=True),
        )
    except (OSError, ValueError, json.JSONDecodeError, SystemExit) as exc:
        errors.append(f"manifest verification input invalid: {exc}")
        matrix, lock, origins = {}, {}, {}

    _validate_manifest_inputs(
        manifest.get("inputs"),
        [
            ("capabilities", capabilities_path, "config/domain-platform-capabilities.json"),
            ("tool_lock", lock_path, "config/tools-lock.json"),
            ("artifact_origins", origins_path, "artifact-origins.json"),
            ("domain_publish_policy", publish_policy_path, "config/domain-publish-policy.json"),
        ],
        errors,
    )
    validate_tool_provenance(root, manifest.get("tools"), lock, errors)
    _validate_manifest_summaries(manifest.get("summaries"), output, errors)
    _validate_manifest_restoration(
        manifest.get("restoration"), manifest.get("build_scope"), baseline, errors
    )
    ...
    for index, entry in enumerate(entries):
        rel = _validate_artifact_entry(entry, index, root, output, matrix, origins, errors)
        if rel is None:
            continue
        if rel in recorded:
    ...
    actual = _collect_actual_artifacts(output, matrix, errors)
    for rel in sorted(actual - recorded):
        errors.append(f"unmanifested artifact: {rel}")
    for rel in sorted(recorded - actual):
        errors.append(f"manifest lists non-publishable artifact: {rel}")
...
def parser() -> argparse.ArgumentParser:
    ...
    create.add_argument("--build-id")
    create.add_argument("--build-scope", choices=("custom", "full"), required=True)
    create.add_argument("--publication-profile", choices=("common", "extended"), required=True)
