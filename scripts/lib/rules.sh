...
compile_domain_binary_dir() {
  ...
  ' sh "$out_dir" "$cache_dir" "$stats_dir" "$compiler" "$output_ext" "$input_ext" < "$list_file" || return 1
...
compile_domain_singbox_json_dir() {
  ...
  compile_domain_binary_dir "$tmp_dir" "$2" "$3" "$cache_dir" '*.json' '.json' sing-box srs "sing-box domain compile"
}

compile_domain_mihomo_text_dir() {
  local tmp_dir="$1"
  local cache_dir="${4:-$tmp_dir/.compile-cache/mihomo}"
  compile_domain_binary_dir "$tmp_dir" "$2" "$3" "$cache_dir" '*.mihomo.txt' '.mihomo.txt' mihomo mrs "mihomo domain compile"
}
