...
build_domain_json_from_rules() {
  ...
  SINGBOX_RULE_SET_VERSION="${SINGBOX_RULE_SET_VERSION:-4}" \
...
compile_domain_binary_dir() {
  ...
  xargs -0 -n 1 -P "$jobs" sh -c '
    ...
    case "$compiler" in
      sing-box) sing-box rule-set compile "$input" --output "$out_dir/$base.$output_ext" ;;
      mihomo) mihomo convert-ruleset domain text "$input" "$out_dir/$base.$output_ext" >/dev/null ;;
    esac
    test -s "$out_dir/$base.$output_ext"
  ' sh "$out_dir" "$compiler" "$output_ext" "$input_ext" < "$list_file" || return 1
  assert_compiled_file_count "$label" "$expected" "$out_dir" "*.$output_ext" || return 1
}

compile_domain_singbox_json_dir() {
  compile_domain_binary_dir "$1" "$2" "$3" '*.json' '.json' sing-box srs "sing-box domain compile"
}

compile_domain_mihomo_text_dir() {
  compile_domain_binary_dir "$1" "$2" "$3" '*.mihomo.txt' '.mihomo.txt' mihomo mrs "mihomo domain compile"
}
