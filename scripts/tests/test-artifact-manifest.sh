...
cp \
  "$ROOT/config/domain-platform-capabilities.json" \
  "$ROOT/config/domain-publish-policy.json" \
...
assert m['publication_profile']=='common'
assert m['inputs']['domain_publish_policy']['path']=='config/domain-publish-policy.json'
PY

DOMAIN_PUBLISH_PROFILE=extended generate
verify
python3 - "$REPO/.output/artifact-manifest.json" <<'PY'
import json, sys
manifest=json.load(open(sys.argv[1], encoding='utf-8'))
assert manifest['publication_profile']=='extended'
PY
generate

cp "$REPO/config/domain-publish-policy.json" "$TMP/domain-publish-policy.json"
printf '\n' >> "$REPO/config/domain-publish-policy.json"
rejects 'domain_publish_policy hash mismatch' verify
cp "$TMP/domain-publish-policy.json" "$REPO/config/domain-publish-policy.json"
