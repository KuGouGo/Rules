...
[ "$(resolve_sing_box_version)" = "1.14.0" ] || fail "sing-box version must come only from lock"
[ "$(resolve_mihomo_version)" = "1.19.30" ] || fail "mihomo version must come only from lock"
[ "$(tool_lock_value sing-box tag_commit)" = "0b8995879f29a9b98ee027bc17b75e101445b238" ] || fail "sing-box tag commit is not locked"
...
PY
write_tool_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box" "sing-box version 1.14.0"
...
if install_tool_with_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box.new" "sing-box version 1.14.0"; then
