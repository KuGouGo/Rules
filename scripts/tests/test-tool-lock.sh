...
[ "$(resolve_sing_box_version)" = "1.13.16" ] || fail "sing-box version must come only from lock"
[ "$(resolve_mihomo_version)" = "1.19.29" ] || fail "mihomo version must come only from lock"
[ "$(tool_lock_value sing-box tag_commit)" = "17ec3c71af8ca946dc50bf0d927c39fc77322aec" ] || fail "sing-box tag commit is not locked"
...
PY
write_tool_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box" "sing-box version 1.13.16"
...
if install_tool_with_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box.new" "sing-box version 1.13.16"; then
