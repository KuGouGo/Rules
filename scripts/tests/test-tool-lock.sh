...
[ "$(resolve_sing_box_version)" = "1.13.20" ] || fail "sing-box version must come only from lock"
[ "$(resolve_mihomo_version)" = "1.19.30" ] || fail "mihomo version must come only from lock"
[ "$(tool_lock_value sing-box tag_commit)" = "56f91dfeabd6f4edbd437dfcc1e5b0ebc856b778" ] || fail "sing-box tag commit is not locked"
...
#!/usr/bin/env bash
printf 'sing-box version 1.13.20\n'
SH
...
PY
write_tool_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box" "sing-box version 1.13.20"
...
if install_tool_with_provenance "sing-box" "linux-amd64" "$BIN_DIR/sing-box.new" "sing-box version 1.13.20"; then
