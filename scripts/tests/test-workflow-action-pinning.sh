...
ACTION_ROOT="${ACTION_ROOT:-$ROOT/.github/actions}"

python3 - "$WORKFLOW_ROOT" "$ACTION_ROOT" <<'PY'
import re
import sys
from pathlib import Path

workflow_root, action_root = map(Path, sys.argv[1:])
workflow_paths = sorted(
    [
        *workflow_root.rglob("*.yml"),
        *workflow_root.rglob("*.yaml"),
        *action_root.rglob("*.yml"),
        *action_root.rglob("*.yaml"),
    ]
)
...
release_comment = re.compile(r"#\s+v[0-9]+\.[0-9]+\.[0-9]+\s*$")
...
for workflow_path in workflow_paths:
    for line_number, line in enumerate(
        ...
        if reference is None or (
            not local_ref.match(reference) and not pinned_ref.fullmatch(reference)
        ):
        ...
        elif not local_ref.match(reference) and not release_comment.search(line):
            errors.append(
                f"{workflow_path}:{line_number}: external action pin must include "
                "its semantic release comment"
            )
