...
cat > "$TMP_DIR/invalid-cidr-google.json" <<'JSON'
{"prefixes":[{"ipv4Prefix":"192.0.2.0/24"},{"ipv6Prefix":"not-a-cidr"}]}
JSON
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single google-json \
  "$TMP_DIR/invalid-cidr-google.json" "$TMP_DIR/invalid-cidr-google.out" \
  >"$TMP_DIR/invalid-cidr-google.stdout" 2>"$TMP_DIR/invalid-cidr-google.stderr"; then
  echo "test failed: invalid google-json CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "prefixes[2] invalid CIDR value: not-a-cidr" "$TMP_DIR/invalid-cidr-google.stderr"; then
  echo "test failed: strict google-json parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-google.stderr" >&2
  exit 1
fi

cat > "$TMP_DIR/invalid-cidr-cloudfront.json" <<'JSON'
{"prefixes":[{"service":"CLOUDFRONT","ip_prefix":"203.0.113.0/24"},{"service":"CLOUDFRONT","ip_prefix":"not-a-cidr"}],"ipv6_prefixes":[{"service":"CLOUDFRONT","ipv6_prefix":"2001:db8::/32"}]}
JSON
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single cloudfront-json \
  "$TMP_DIR/invalid-cidr-cloudfront.json" "$TMP_DIR/invalid-cidr-cloudfront.out" \
  >"$TMP_DIR/invalid-cidr-cloudfront.stdout" 2>"$TMP_DIR/invalid-cidr-cloudfront.stderr"; then
  echo "test failed: invalid cloudfront-json CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "prefixes[2] invalid CIDR value: not-a-cidr" "$TMP_DIR/invalid-cidr-cloudfront.stderr"; then
  echo "test failed: strict cloudfront-json parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-cloudfront.stderr" >&2
  exit 1
fi

cat > "$TMP_DIR/cloudfront-filter.json" <<'JSON'
{"prefixes":[{"service":"AMAZON","ip_prefix":"192.0.2.0/24"},{"service":"CLOUDFRONT","ip_prefix":"198.51.100.0/24"}],"ipv6_prefixes":[]}
JSON
python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single cloudfront-json \
  "$TMP_DIR/cloudfront-filter.json" "$TMP_DIR/cloudfront-filter.out"
assert_file_content "$TMP_DIR/cloudfront-filter.out" $'198.51.100.0/24'

cat > "$TMP_DIR/invalid-cidr-fastly.json" <<'JSON'
{"addresses":["203.0.113.0/24","not-a-cidr"],"ipv6_addresses":[]}
JSON
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single fastly-json \
  "$TMP_DIR/invalid-cidr-fastly.json" "$TMP_DIR/invalid-cidr-fastly.out" \
  >"$TMP_DIR/invalid-cidr-fastly.stdout" 2>"$TMP_DIR/invalid-cidr-fastly.stderr"; then
  echo "test failed: invalid fastly-json CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "addresses[2] invalid CIDR value: not-a-cidr" "$TMP_DIR/invalid-cidr-fastly.stderr"; then
  echo "test failed: strict fastly-json parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-fastly.stderr" >&2
  exit 1
fi

cat > "$TMP_DIR/invalid-cidr-ripe.json" <<'JSON'
{"data":{"prefixes":[{"prefix":"203.0.113.0/24"},{"prefix":"not-a-cidr"}]}}
JSON
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single ripe-stat-json \
  "$TMP_DIR/invalid-cidr-ripe.json" "$TMP_DIR/invalid-cidr-ripe.out" \
  >"$TMP_DIR/invalid-cidr-ripe.stdout" 2>"$TMP_DIR/invalid-cidr-ripe.stderr"; then
  echo "test failed: invalid ripe-stat-json CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "prefixes[2] invalid CIDR value: not-a-cidr" "$TMP_DIR/invalid-cidr-ripe.stderr"; then
  echo "test failed: strict ripe-stat-json parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-ripe.stderr" >&2
  exit 1
fi
