...
printf '%s\n' '<li>203.0.113.0/99</li>' > "$TMP_DIR/invalid-cidr.html"
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single html \
  "$TMP_DIR/invalid-cidr.html" "$TMP_DIR/invalid-cidr-html.out" \
  >"$TMP_DIR/invalid-cidr-html.stdout" 2>"$TMP_DIR/invalid-cidr-html.stderr"; then
  echo "test failed: invalid html CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "CIDR match #1 invalid CIDR value: 203.0.113.0/99" "$TMP_DIR/invalid-cidr-html.stderr"; then
  echo "test failed: strict html parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-html.stderr" >&2
  exit 1
fi

cat > "$TMP_DIR/invalid-cidr-aws.json" <<'JSON'
{"prefixes":[{"ip_prefix":"203.0.113.0/24"},{"ip_prefix":"not-a-cidr"}],"ipv6_prefixes":[]}
JSON
if python3 "$ROOT/scripts/tools/normalize-ip-rules.py" single aws-json \
  "$TMP_DIR/invalid-cidr-aws.json" "$TMP_DIR/invalid-cidr-aws.out" \
  >"$TMP_DIR/invalid-cidr-aws.stdout" 2>"$TMP_DIR/invalid-cidr-aws.stderr"; then
  echo "test failed: invalid aws-json CIDR value should fail" >&2
  exit 1
fi
if ! grep -Fq "prefixes[2] invalid CIDR value: not-a-cidr" "$TMP_DIR/invalid-cidr-aws.stderr"; then
  echo "test failed: strict aws-json parser did not identify invalid CIDR" >&2
  cat "$TMP_DIR/invalid-cidr-aws.stderr" >&2
  exit 1
fi
