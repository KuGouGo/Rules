...
assert_scope full "pull_request includes changes outside custom sources" \
  EVENT_NAME=pull_request CURRENT_SHA=HEAD BEFORE_SHA=base \
  CHANGED_FILES=$'config/upstream-pins.json'
...
assert_scope full "publication cohort inconsistent; using full sync" \
  RULES_PUBLICATION_BASELINE_INPUT="$DEAD_BASELINE_INPUT" \
  EVENT_NAME=push CURRENT_SHA=HEAD BEFORE_SHA="$MIDDLE_SOURCE" \
  CHANGED_FILES=$'sources/custom/domain/custom.list'
grep -Fq '"status": "inconsistent"' "$BASELINE_OUTPUT" || {
  echo "test failed: dead-source baseline was not persisted as inconsistent" >&2
  cat "$BASELINE_OUTPUT" >&2
  exit 1
}
grep -Fq '"generation_id": null' "$BASELINE_OUTPUT" || {
  echo "test failed: dead-source baseline kept a cohort identity" >&2
  cat "$BASELINE_OUTPUT" >&2
  exit 1
}
...
git -C "$SCOPE_REPO" add orphan.txt
git -C "$SCOPE_REPO" commit -q -m orphan
ORPHAN_SOURCE="$(git -C "$SCOPE_REPO" rev-parse HEAD)"
git -C "$SCOPE_REPO" checkout -q "$SCOPE_ORIG_BRANCH"
ORPHAN_BASELINE_INPUT="$TEST_TMP/publication-baseline-orphan-source.json"
write_baseline "$ORPHAN_BASELINE_INPUT" "$ORPHAN_SOURCE"
assert_scope full "publication cohort inconsistent; using full sync" \
  RULES_PUBLICATION_BASELINE_INPUT="$ORPHAN_BASELINE_INPUT" \
  EVENT_NAME=push CURRENT_SHA=HEAD BEFORE_SHA="$MIDDLE_SOURCE" \
  CHANGED_FILES=$'sources/custom/domain/custom.list'
grep -Fq '"status": "inconsistent"' "$BASELINE_OUTPUT" || {
  echo "test failed: orphan-source baseline was not persisted as inconsistent" >&2
  cat "$BASELINE_OUTPUT" >&2
  exit 1
}
