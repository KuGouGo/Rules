...
DOMAIN-SUFFIX,example.com
EOF
  cat > "$TMP_DIR/suffix_compaction/base_cn.expected" <<'EOF'
DOMAIN,exact.example.com
EOF
  cat > "$TMP_DIR/suffix_compaction/base_not_cn.expected" <<'EOF'
...
EOF
  ...
  assert_file_equals \
    "$TMP_DIR/suffix_compaction/base_cn.expected" \
    "$TMP_DIR/suffix_compaction/out/base@cn.list" \
    "@cn derivative keeps rules not covered inside that derivative"
  assert_file_equals \
    "$TMP_DIR/suffix_compaction/base_not_cn.expected" \
    "$TMP_DIR/suffix_compaction/out/base@!cn.list" \
    "@!cn derivative keeps rules not covered inside that derivative"
}
...
assert manifest["by_attr"] == {"!cn": 2, "cn": 4}
