...
if dlc.get("url") != "https://github.com/nekolsd/dlc2.git":
    raise SystemExit("test failed: domain.dlc URL must point at the dlc2 curated domain list")
if "loyalsoldier-china-list" in json.loads(Path("config/upstreams.json").read_text(encoding="utf-8"))["domain"]:
