...
expected = ["cn", "private", "google", "telegram", "cloudflare", "cloudfront", "fastly", "apple"]
...
apple_builtin_start = script.index('"$BUILTIN_APPLE_SOURCE_FILE"')
apple_builtin_body = script[apple_builtin_start : apple_builtin_start + 300]
if "apple.cidr.txt" not in apple_builtin_body:
    raise SystemExit("test failed: Apple IP must be derived from the built-in source")
