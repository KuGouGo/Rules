...
for branch in "${BRANCHES[@]}"; do
  ...
  parent_commit="$(git --git-dir="$REMOTE" rev-parse --verify "$branch^" 2>/dev/null || true)"
  ...
  [ -z "$parent_commit" ] || {
    echo "test failed: $branch publication commit must be an orphan rebuild without parent history" >&2
    exit 1
  }
