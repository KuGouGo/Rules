...
TOOL = _paths.TOOLS_DIR / "verify-domain-derivatives.py"


def build_manifest(required=True, geolocation_pair=True, aggregate_rules=600):
    lists = [
    ...
    region_pairs = {"geolocation": ["!cn"]} if geolocation_pair else {}
    return {"lists": lists, "region_pairs": region_pairs}


class VerifyDomainDerivativesTest(unittest.TestCase):
    def run_tool(self, manifest_path):
        return subprocess.run(
            [sys.executable, str(TOOL), str(manifest_path), "500"],
            capture_output=True,
            text=True,
        )
