...
RULES_TRACE_DOMAIN_CLI_FILE="$TRACE_BUILD" \
  ...
  ./scripts/commands/build-custom-rules.sh >/dev/null
...
lint_line="$(grep -nF "$lint_invocation" "$ROOT/scripts/commands/build-custom-rules.sh" | cut -d: -f1)"
tmp_line="$(grep -nF "$tmp_setup" "$ROOT/scripts/commands/build-custom-rules.sh" | cut -d: -f1)"
setup_line="$(grep -nF 'setup_tool_cache' "$ROOT/scripts/commands/build-custom-rules.sh" | cut -d: -f1)"
