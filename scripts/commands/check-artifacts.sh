...
MIN_IP_CIDR_APPLE="${MIN_IP_CIDR_APPLE:-10}"
MIN_IP_CIDR_PRIVATE="${MIN_IP_CIDR_PRIVATE:-15}"
MIN_DOMAIN_ARTIFACT_FILES="${MIN_DOMAIN_ARTIFACT_FILES:-40}"
MAX_DOMAIN_ARTIFACT_FILES="${MAX_DOMAIN_ARTIFACT_FILES:-90}"
...
check_no_redundant_attr_filter_artifacts() {
  local platform
  while IFS=$'\t' read -r platform _public _branch section _extension _format _compiler; do
...
check_public_ip_cidrs_in_dir() {
  ...
  python3 "$ROOT/scripts/tools/check_ip_cidr_validity.py" "$dir" "$label"
}
...
main() {
  ...
  while IFS=$'\t' read -r platform _public _branch section extension _format _compiler; do
    if [ "$section" = domain ]; then min_expected="$MIN_DOMAIN_ARTIFACT_FILES"; else min_expected=4; fi
