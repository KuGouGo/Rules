...
IP_TEXT_ARTIFACTS=(cn private google telegram cloudflare aws fastly apple)
...
entries = {
    ...
    "domain.loyalsoldier-china-list.url": config["domain"]["loyalsoldier-china-list"]["url"],
    ...
    "ip.aws.url": config["ip"]["aws"]["url"],
    "ip.fastly.url": config["ip"]["fastly"]["url"],
    "ip.apple.url": config["ip"]["apple"]["url"],
    "ip.apple.fallback_url": config["ip"]["apple"]["fallback_url"],
...
require_upstream_settings \
  domain.dlc.url domain.loyalsoldier-china-list.url \
  domain.shellcrash-fakeip.url \
  ip.cn-ipv46.url ip.cn-ipv46-apnic.url \
  ip.google.url ip.telegram.url ip.cloudflare-ipv4.url ip.cloudflare-ipv6.url \
  ip.aws.url ip.fastly.url \
  ip.apple.url ip.apple.fallback_url ip.ripe-stat.base_url \
  asn.telegram
...
AWS_IP_SOURCE_URL="${UPSTREAM_SETTINGS[ip.aws.url]}"
FASTLY_IP_SOURCE_URL="${UPSTREAM_SETTINGS[ip.fastly.url]}"
APPLE_IP_SOURCE_URL="${UPSTREAM_SETTINGS[ip.apple.url]}"
APPLE_IP_SOURCE_FALLBACK_URL="${UPSTREAM_SETTINGS[ip.apple.fallback_url]}"
...
assert_domain_attr_derivatives() {
  ...
  python3 "$ROOT_DIR/scripts/tools/assert-domain-derivatives.py" \
    ...
    "$DLC_MIN_NOT_CN_ATTR_RULESETS" "$DLC_MIN_ADS_ATTR_RULESETS" \
    "$DLC_MIN_REGIONAL_RULESETS"
}
...
main() {
  ...
  download_files_parallel \
    ...
    aws required "$AWS_IP_SOURCE_URL" "$IP_BUILD_TMP_DIR/aws.raw.json" \
