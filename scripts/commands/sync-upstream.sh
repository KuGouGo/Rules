...
main() {
  ...
  asn_v4_lines="$(wc -l < "$IP_BUILD_TMP_DIR/geoip_asn_v4.raw.csv")"
  asn_v6_lines="$(wc -l < "$IP_BUILD_TMP_DIR/geoip_asn_v6.raw.csv")"
  if [ "$asn_v4_lines" -lt 600000 ] || [ "$asn_v6_lines" -lt 380000 ]; then
    echo "GeoLite2-ASN CSV health gate failed: ipv4=${asn_v4_lines} lines, ipv6=${asn_v6_lines} lines" >&2
    exit 1
  fi
...
EOF
  ...
  if [ "${#TELEGRAM_ASNS[@]}" -gt 0 ]; then
    sync_pure_asn_ip_list telegram "${TELEGRAM_ASNS[@]}"
  fi
  if [ "${#GOOGLE_ASNS[@]}" -gt 0 ]; then
    sync_pure_asn_ip_list google "${GOOGLE_ASNS[@]}"
  fi
