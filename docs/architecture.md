...
- **域名**：v2fly/domain-list-community（核心树）、Loyalsoldier `china-list`（DNS 分流）、ShellCrash `fakeip-filter`。
- **IP**：cn-ipv46、google、telegram、cloudflare、aws、fastly、apple 等，以及由 RIPEstat 按 ASN 拉取的前缀。
- **本地**：`sources/custom/` 自定义规则与 `sources/builtin/` 内置保留地址。

## 规范化与合并

- 统一大小写与书写形式，按 `DOMAIN` / `DOMAIN-SUFFIX` 超集折叠去重。
- 保留 DLC 的 `@cn` / `@!cn` / `@ads` 属性并派生对应规则集（如 `geolocation-!cn@cn`）。
...
发布前执行守卫（`guard-artifacts.sh`）：产物数量下限、地址族与保留地址校验、必需派生集完整性检查。全部通过后，产物在事务内原子切换，失败自动回滚并保留诊断信息。
